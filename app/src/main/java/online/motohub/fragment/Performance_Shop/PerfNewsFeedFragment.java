...
public class PerfNewsFeedFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private ClubPostsAdapter mPromoterPostsAdapter;
    ...
    private void initView() {
        ...
        try {
            mPromoterPostsAdapter = new ClubPostsAdapter(mNewsFeedList, mPromotersResModel, mMyProfileResModel, mActivity);
    ...
    private void updatePost(Intent intent) {
        ...
        showSnackBar(mParent, getString(R.string.post_update_success));
    }
    ...
    private void getNewsFeedPosts() {
        ...
        String mFilter = "((ProfileID=" + mPromotersResModel.getUserId() + ") AND (user_type=shop)) OR ((" + PostsModel.TO_SUBSCRIBED_USER_ID + "=" + mPromotersResModel.getUserId() + ") AND (user_type = shop_user)) AND (ReportStatus == 0)";
        ...
        if (responseObj instanceof PostsModel) {
            ...
            switch (responseType) {
                ...
                case RetrofitClient.DELETE_SHARED_POST_RESPONSE:
                    RetrofitClient.getRetrofitInstance().callDeleteProfilePosts((BaseActivity) mActivity, mNewsFeedList.get(mCurrentPostPosition).getID(), RetrofitClient.DELETE_PROFILE_POSTS_RESPONSE);
            }
