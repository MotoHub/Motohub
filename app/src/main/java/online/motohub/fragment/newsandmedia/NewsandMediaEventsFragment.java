...
public class NewsandMediaEventsFragment extends BaseFragment {
    ...
    public void alertDialogPositiveBtnClick(String dialogType, int position) {
        ...
        switch (dialogType) {
            case AppDialogFragment.ALERT_API_FAILURE_DIALOG:
                switch (mFailureResponseType) {
                    case RetrofitClient.EVENTS_BOOKING_RESPONSE:
                        mClubEventsAdapter.bookAnEventRequest(mTempPaymentModel);
                        break;
                    case RetrofitClient.RACING_RESPONSE:
                        mClubEventsAdapter.callPostRacingData();
                        break;
                }
                break;
            case AppDialogFragment.ALERT_SPECTATOR_UPDATE_DIALOG:
                ...
                break;
            default:
                mClubEventsAdapter.alertDialogPositiveBtnClick(dialogType);
                ((BaseActivity) mActivity).dismissAppDialog();
        }
    }
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case EventsFindAdapter.EVENT_PAYMENT_REQ_CODE:
                    mClubEventsAdapter.onActivityResult(requestCode, resultCode, data);
                    break;
    ...
    public void retrofitOnResponse(Object responseObj, int responseType) {
        ...
        if (responseObj instanceof EventsModel) {
            ...
            if (mEventsModel.getResource() != null && mEventsModel.getResource().size() > 0) {
                mClubEventsListData.clear();
                mClubEventsListData.addAll(mEventsModel.getResource());
                mClubEventsAdapter.notifyDataSetChanged();
                mRefresh = false;
            } else
            ...
            if (mEventsWhoIsGoingModel.getResource() != null && mEventsWhoIsGoingModel.getResource().size() > 0) {
                EventsInterface mEventsInterface = mClubEventsAdapter;
            ...
            mClubEventsAdapter.bookAnEventRequest(mPaymentModel);
            ...
            mClubEventsAdapter.callUpdateEventAnswerScreen(mResEventAnswerModel);
        }
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser && mClubEventsListData.size() == 0)
            callGetEvents();
    }
}
