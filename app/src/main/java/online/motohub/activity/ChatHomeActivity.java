...
public class ChatHomeActivity extends BaseActivity implements TabLayout.OnTabSelectedListener {
    ...
    private void initViews() {
        ...
        if (!isNetworkConnected(ChatHomeActivity.this)) {
            Toast.makeText(ChatHomeActivity.this, getString(R.string.internet_err), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        switch (v.getId()) {
            ...
            case R.id.create_chat:
                if (mChatTabLayout.getSelectedTabPosition() == 0) {
                    ((BaseFragment) mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition()))
                            .launchSingleChatAddActivity();
                } else {
                    ((BaseFragment) mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition()))
                            .launchCreateGroupChatActivity();
                }
                break;
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ADD_NEW_SINGLE_CHAT) {
            if (resultCode == RESULT_OK) {
                ...
                //ProfileResModel mSelectedProfile = MotoHub.getApplicationInstance().getmOthersProfileResModel();
                ProfileResModel mSelectedProfile = EventBus.getDefault().getStickyEvent(ProfileResModel.class);
                ...
                RetrofitClient.getRetrofitInstance()
                        .callCreateSingleChatRoom(this, mSingleChatRoomModel, RetrofitClient.CREATE_SINGLE_CHAT_ROOM);
            }
        } else if (requestCode == ADD_NEW_GROUP_CHAT) {
            if (resultCode == RESULT_OK) {
                GroupChatRoomResModel mGroupChatRoomResModel = (GroupChatRoomResModel) data.getExtras()
                        .get(GroupChatRoomModel.GRP_CHAT_ROOM_RES_MODEL);
                ((BaseFragment) mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition()))
                        .callUpdateGroupChatRoom(mGroupChatRoomResModel);
            }
        } else if (requestCode == RetrofitClient.GET_SINGLE_CHAT_COUNT) {
            if (resultCode == RESULT_OK) {
                mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition())
        ...
        if (responseObj instanceof SingleChatRoomModel) {
            ...
            switch (responseType) {
                ...
                case RetrofitClient.CREATE_SINGLE_CHAT_ROOM:
                    /*((BaseFragment) mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition()))
                            .getaddedChat(MotoHub.getApplicationInstance().getmProfileResModel(), true);*/
                    ((BaseFragment) mChatTabPagerAdapter.getItem(mChatTabLayout.getSelectedTabPosition()))
                            .getaddedChat(EventBus.getDefault().getStickyEvent(ProfileResModel.class), true);
            ...
            switch (responseType) {
                case RetrofitClient.GET_SINGLE_CHAT_COUNT:
                    ((BaseFragment) mChatTabPagerAdapter
                            .getItem(mChatTabLayout.getSelectedTabPosition())).retrofitOnResponse(responseObj, responseType);
                    break;
            }
