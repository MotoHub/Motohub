...
public class VideoCommentsActivity extends BaseActivity implements TaggedProfilesAdapter.TaggedProfilesSizeInterface {
    ...
    private VideoCommentsAdapter mFeedCommentsAdapter;
    private ArrayList<VideoCommentsModel> mFeedCommentList = new ArrayList<>();
    ...
    protected void onRestoreInstanceState(@NonNull Bundle savedInstanceState) {
        ...
        ArrayList<VideoCommentsModel> mTempCommentList = (ArrayList<VideoCommentsModel>)
                savedInstanceState
    ...
    private void initView() {
        ...
        if (getIntent().getExtras().getBoolean(MyFireBaseMessagingService.IS_FROM_NOTIFICATION_TRAY)) {
            try {
                ...
                mPostID = mDetailsObj.getInt("VideoCommentID");
                mFilter = "VideoID=" + mPostID;
                getMotoProfiles();
                return;
            } catch (Exception e) {
                e.printStackTrace();
        ...
        mPostID = getIntent().getIntExtra(AppConstants.VIDEO_ID, 0);
        mFilter = "VideoID=" + mPostID;
        ...
        switch (v.getId()) {
            ...
            case R.id.post_btn:
                ...
                if (mPostImgUri != null) {
                    ...
                    try {
                        callPostVideoComments(imgUrl, mPostID, mMyProfileResModel.getID());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    private void callPostVideoComments(String imgUrl, int mPostId, int mProfileId) throws Exception {
        ...
        mItem.addProperty("VideoID", mPostId);
        ...
        RetrofitClient.getRetrofitInstance().callPostVideoComments(this, mJsonObject, RetrofitClient.VIDEO_COMMENTS);
    ...
    private void getComments(String mFilter) {
        RetrofitClient.getRetrofitInstance().callGetVideoComments(this, mFilter, RetrofitClient.CALL_GET_VIDEO_COMMENTS);
    }
    ...
    public void retrofitOnResponse(Object responseObj, int responseType) {
        ...
        if (responseObj instanceof ProfileModel) {
            ...
            VideoCommentsModel mResFeedCommentModel = (VideoCommentsModel) responseObj;
            switch (responseType) {
                case RetrofitClient.VIDEO_COMMENTS:
                    ...
                    break;
                case RetrofitClient.CALL_GET_VIDEO_COMMENTS:
                    mFeedCommentList = mResFeedCommentModel.getResource();
                    setFeedCommentAdapter();
                    break;
            }
        } else if (responseObj instanceof VideoCommentLikeModel) {
            VideoCommentLikeModel mFeedCommentLikeList = (VideoCommentLikeModel) responseObj;
            ArrayList<VideoCommentLikeModel> mCommentLike = mFeedCommentLikeList.getResource();
            switch (responseType) {
                case RetrofitClient.VIDEO_COMMENTS_LIKE:
                    if (mCommentLike != null && mCommentLike.size() > 0)
                        mFeedCommentsAdapter.resetCommentsLikeList(mCommentLike.get(0));
                    break;
                case RetrofitClient.VIDEO_COMMENTS_UNLIKE:
                    if (mCommentLike != null && mCommentLike.size() > 0)
    ...
    private void setFeedCommentAdapter() {
        setCommentList();
        mFeedCommentsAdapter = new VideoCommentsAdapter(this, mFeedCommentList, mMyProfileResModel);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                ...
                case AppConstants.VIDEO_COMMENT_REPLY_REQUEST:
                    assert data.getExtras() != null;
                    ArrayList<VideoCommentReplyModel> mFeedCommentModel = (ArrayList<VideoCommentReplyModel>) data.getExtras().getSerializable(AppConstants.COMMENT_REPLY_FOR_POST_VIDEOS);
    ...
    public void addTaggedFriends(ProfileResModel profileResModel) {
        ...
        mCommentTaggedUserList.add("@" + Utility.getInstance().getUserName(profileResModel));
        mCommentTaggedUserDetails.put(profileResModel.getUserID(), "@" + Utility.getInstance().getUserName(profileResModel));
        if (mCommentTaggedUserNames.trim().isEmpty()) {
            mCommentTaggedUserNames = "@" + Utility.getInstance().getUserName(profileResModel);
        } else {
