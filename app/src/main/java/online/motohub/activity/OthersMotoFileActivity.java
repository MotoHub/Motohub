...
public class OthersMotoFileActivity extends BaseActivity implements
    ...
    private void setPostAdapter() {
        if (mPostsAdapter == null) {
            mPostsAdapter = new PostsAdapter(mPostsList,  mMyProfileResModel, this);
    ...
    private void setProfile() {
        ...
        if (Utility.getInstance().isSpectator(mOtherProfileResModel)) {
            mNameOfMotoTv.setText(mOtherProfileResModel.getSpectatorName());
            mNameOfDriverTv.setVisibility(View.GONE);
        } else {
            mNameOfMotoTv.setText(mOtherProfileResModel.getMotoName());
            mNameOfDriverTv.setVisibility(View.VISIBLE);
            mNameOfDriverTv.setText(mOtherProfileResModel.getDriver());
        }
    }
    ...
    private void getProfilePosts() {
        ...
        if (mBlockedUsers.trim().isEmpty()) {
            mFilter = "((ProfileID=" + mOtherProfileResModel.getID()
                    + ") OR (TaggedProfileID LIKE '%" + mOtherProfileResModel.getID()
                    + "%')) AND (IsNewsFeedPost=" + true + ") AND ((user_type!='promoter') AND " +
                    "(user_type!='club') AND (user_type!='newsmedia') AND (user_type!='track') AND (user_type!='shop'))";
        } else {
            mFilter = "((ProfileID=" + mOtherProfileResModel.getID()
                    + ") OR (TaggedProfileID LIKE '%" + mOtherProfileResModel.getID()
                    + "%')) AND (ProfileID NOT IN (" + mBlockedUsers + ")) AND (IsNewsFeedPost="
                    + true + ") AND ((user_type!='promoter') AND (user_type!='club') AND (user_type!='newsmedia') AND (user_type!='track')AND (user_type!='shop'))";
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.follow_un_follow_box:
                ...
                if (mIsAlreadyBlocked) {
                    showToast(this, getString(R.string.follow_err));
                } else if (isBlockedMe) {
                    showToast(this, getString(R.string.block_follow_err));
                } else {
                    if (mIsAlreadyFollowing) {
                        ...
                        CommonAPI.getInstance().callFollowProfile(this, mRetrofitResInterface, mMyProfileID, mOtherProfileID);
                    }
                }
                break;
            case R.id.block_box:
            ...
            case R.id.chat_box:
                if (mSingleChatRoomResModel == null) {
                    ...
                    RetrofitClient.getRetrofitInstance().callGetSingleChatRoom(this, mFilter, RetrofitClient.GET_SINGLE_CHAT_ROOM);
                } else {
                    showSingleChatBox();
                }
                break;
            ...
            case R.id.live_box:
                if (isStartLiveEnable()) {
                    DialogManager.showLiveOptionPopup(this, mCommonReturnInterface);
                } else {
                    viewLiveStream();
                }
...
//            showSnackBar(mCoordinatorLayout, mUnFollowSuccess);
        ...
        int followingCount = mOtherProfileResModel.getFollowprofile_by_ProfileID().size();
        mFollowingCount.setText(String.valueOf(followingCount));
        int followersCount = mOtherProfileResModel.getFollowprofile_by_FollowProfileID().size();
        mFollowersCount.setText(String.valueOf(followersCount));
    }

    private void setResult() {
        setFollowUnFollowContent();
        setResult(RESULT_OK, new Intent().putExtra(AppConstants.MY_PROFILE_OBJ, mMyProfileResModel)
    ...
    private void showSingleChatBox() {
        ...
        mBundle.putSerializable(ProfileModel.MY_PROFILE_RES_MODEL, mMyProfileResModel);
        startActivity(new Intent(this, ChatBoxSingleActivity.class).putExtras(mBundle));
    }
        ...
        if (responseObj instanceof ProfileModel) {
            ...
            switch (responseType) {
                case RetrofitClient.POST_SHARES:
                    ...
                    CommonAPI.getInstance().callAddSharedPost(this,mPostsList.get(mCurrentPostPosition),mMyProfileResModel);
                    break;
            }
    ...
    public void alertDialogPositiveBtnClick(BaseActivity activity, String dialogType, StringBuilder profileTypesStr, ArrayList<String> profileTypes, int position) {
        ...
        switch (dialogType) {
            ...
            case AppDialogFragment.BOTTOM_VIEW_PHOTOS_DIALOG:
                ...
                photoIntent.putExtra(ProfileImgGalleryActivity.EXTRA_PROFILE, mOtherProfileResModel);
            ...
            case AppDialogFragment.BOTTOM_VIEW_VIDEOS_DIALOG:
                ...
                videoIntent.putExtra(ProfileVideoGalleryActivity.EXTRA_PROFILE, mOtherProfileResModel);
