...
import online.motohub.activity.ondemand.PromotorOrUserVideoPlaying;
...
public class PromoterOrUserAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
            ...
            if (type == 2) {
                ...
                mActivity.startActivityForResult(
                        new Intent(mActivity, ReportActivity.class)
                                ...
                                .putExtra(ProfileModel.USER_ID, mCurrentProfileResModel.getUserID()).putExtra(AppConstants.REPORT, AppConstants.REPORT_VIDEO),
    ...
    public PromoterOrUserAdapter(ArrayList<PromoterVideoModel.Resource> postsList, ProfileResModel mMyProfileResModel, Context ctx, int ProfileID, PromoterOrUserFragment promoterOrUserFragment) {
            ...
            case VIEW_TYPE_POSTS:
                ...
                try {
                    ...
                    if (mModel.getCaption() == null || mModel.getCaption().isEmpty()) {
                        ...
                        //mViewHolderPost.mPostText.setText(replacer(sb.append(mModel.getCaption())));
                ...
                mViewHolderPost.mPostImageVideoBox.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (mPostsList != null && isNetwork() && mCurrentProfileResModel != null) {
                            ...
                            ((OnDemandActivity) mContext).startActivityForResult(intent, AppConstants.ONDEMAND_REQUEST);
                        } else {
        ...
        if (!mCommentImgUrl.isEmpty()) {
            ...
            try {
                if (mCommentText.contains(" ")) {
                    mViewHolderPost.mCommentTxt.setText(((BaseActivity) mContext).setTextEdt(mContext, mCommentText, mFeedCommentModel.get(latestPostion).getCommentTaggedUserNames(), mFeedCommentModel.get(latestPostion).getCommentTaggedUserID(), mCurrentProfileResModel.getID()));
                } else {
                ...
                }//mViewHolderPost.mCommentTxt.setText(((BaseActivity) mContext).setTextEdt(mContext, (replacer(sb.append(mCommentText))), mFeedCommentModel.get(latestPostion).getCommentTaggedUserNames(), mFeedCommentModel.get(latestPostion).getCommentTaggedUserID(), mCurrentProfileResModel.getID()));
            } catch (Exception e) {
                e.printStackTrace();
            }
            mViewHolderPost.mCommentTxt.setVisibility(View.VISIBLE);
    ...
    private void callUnLikePost(Integer profileId) {
        RetrofitClient.getRetrofitInstance().callUnLikeForVideos(promoterOrUserFragment, profileId, RetrofitClient.VIDEO_UNLIKE);
    }
    ...
    private void callLikePost(int postId, int profileId) {
        ...
        RetrofitClient.getRetrofitInstance().postLikesForVideos(promoterOrUserFragment, mJsonObject, RetrofitClient.VIDEO_LIKES);
    }
    ...
    private void navigationToProfileActivity(int layoutPosition) {
        ...
        if (mPostsList.get(layoutPosition).getPromoter_by_UserID() != null) {
            Class mClassName;
            switch (mUserType) {
            ...
            /*mBundle.putSerializable(PromotersModel.PROMOTERS_RES_MODEL, mPostsList.get(layoutPosition).getPromoter_by_UserID());
            ...
            /*MotoHub.getApplicationInstance().setmProfileResModel(mCurrentProfileResModel);
            MotoHub.getApplicationInstance().setmPromoterResModel(mPostsList.get(layoutPosition).getPromoter_by_UserID());*/
            EventBus.getDefault().postSticky(mCurrentProfileResModel);
            EventBus.getDefault().postSticky(mPostsList.get(layoutPosition).getPromoter_by_UserID());
            ((BaseActivity) mContext).startActivityForResult(
            ...
            switch (view.getId()) {
                case R.id.circular_img_view:
                    if (mCurrentProfileResModel != null && mPostsList.size() > 0 && mPostsList != null) {
                        try {
                            if (mPostsList.get(selPos).getUserType().equals(AppConstants.ONDEMAND) || mPostsList.get(selPos).getUserType().equals(AppConstants.USER)) {
